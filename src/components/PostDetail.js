...
class PostDetail extends Component{
    ...
    handleImageChange(e){
        this.setState({maxFileError:false})
    ...
    addCategory(){
        var new_category_name=window.$("input[name='new_category']").val()
        var self=this
    ...
    handleChange(e){ 
        ...
        if((this.state.categories.filter((x)=>x.category.toLowerCase() === e.target.value.toLowerCase())).length > 0){
            window.$('.new_category_help').text("Already Exist This Category").css('display','').css('color',"red")
            window.$('.add_category_btn').css('display', '')
            window.$('.add_category_btn_new').css('display', 'none')

        }
        else if(e.target.value == ''){
            window.$('.new_category_help').text("Please Enter Valid Category").css('display','').css('color',"red")
            window.$('.add_category_btn').css('display', '')
            window.$('.add_category_btn_new').css('display', 'none')
        }
        else{
            window.$('.add_category_btn').css('display', 'none')
            window.$('.new_category_help').css('display','none');
            window.$('.add_category_btn_new').css('display', '')
    ...
    handleDeletePost(e){
     ...
     OpenModel(e){
        window.$('#delete_input_id').val(e.target.id.split('_')[2])
        window.$("#deletePost").modal('show');
    }
                    ...
                    <div className="col-md-8 offset-md-2">
                        ...
                        <div className="content-section">
                            ...
                            <fieldset className="form-group">
                                ...
                                {this.state.maxFileError &&
                                <h6 className="text-center text-danger">File Size Exceed Limit! Please Try with small image size</h6>
                                    }
                                ...
                                <button type="button" href="#" className="btn btn-primary active" role="button" data-target="#exampleModal" data-toggle="modal" aria-pressed="true"><i data-toggle="tooltip" data-placement="top" title="Add New Category" className="fas fa-arrow-up"></i></button>
                                    ...
                                    <div className="row mt-4 text-center">
                                        <div className="col-6">
                                        {this.state.error || this.state.maxFileError?
                                            <button type="button" className="btn btn-success" disabled="disabled">Update Post</button>
                                        :
