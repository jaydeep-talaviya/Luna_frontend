...
class PostFilterProfile extends Component {
    ...
    handleFocus(e){
        window.$('#hint_id_password1').removeClass('d-none')
    }
    ...
    handleClear() {
        ...
        window.$('.clear_btn').addClass('d-none');
    }
    ...
    handleChangeInputs(event) {
        ...
        if(event.target.name=='password'){
            if(event.target.value.length>=8){
                window.$('#hint_id_password1').addClass('d-none')
        ...
        if (event.target.name == 'confirmpassword' || event.target.name=='password') {
            if (this.state.password == '') {
                window.$('.password_help').text("please enter password").css('display','').css('color',"red")
            }
            else{
                if (confirmpassword === password && (confirmpassword.length > 8 && password.length > 8)) {
                    window.$('.password_help').text("Matched password").css('display','').css('color',"green")

                }
                else {
                    window.$('.password_help').text("please enter Valid password").css('display','').css('color',"red")
    ...
    handleSelectCategory(e) {
        ...
        window.$('.clear_btn').removeClass('d-none');
    ...
    render() {
        ...
        const is_profile = this.state.user.id == user_id
        return (
            <div>
                {this.state.error ?
                ...
                <div className="row">
                    ...
                    {isSmallerThenTablet ?
                        <div className="col-md-8" id="style-15">

                            <div className="card">
                                {this.state.allcategory.length>0?
                                 this.state.allpost.map(h =>
                                    <SinglePost key={h.id} postdata={h} user_id={Number(user_id)} is_profile={is_profile} history={this.props.history} />
                                )
                                :
                        ...
                        :
                        <div className="col-md-8" style={{ height: '80vh', overflow: 'scroll' }} id="style-15">

                            <div className="card">
                            {this.state.allcategory.length>0?
                                this.state.allpost.map(h =>
                                    <SinglePost key={h.id} postdata={h} user_id={Number(user_id)} is_profile={is_profile} />
                                )
                                :
                                <h3 className='text-center'>create Your Own Post</h3>
                                }
